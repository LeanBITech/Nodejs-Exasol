...
module.exports = {
  Exasol: function (url, user, pass, autocommit, onconnect, onerror) {
    var context = this;
    ...
    var Hex = {
      ...
      encode: function ($input) {
        ...
        do {
          ...
          $output +=
            this.hex.charAt(($k >> 4) & 0xf) + this.hex.charAt($k & 0xf);
        } while ($i < $input.length);
        return $output;
      },
      decode: function ($input) {
        ...
        do {
          $output += String.fromCharCode(
            ((this.hex.indexOf($input.charAt($i++)) << 4) & 0xf0) |
    ...
    var pw_encode = function (pass, exp, mod) {
      ...
      if ((encdata.length & 1) != 0) encdata = "0" + encdata;
    ...
    context.com = function (request, onresponse, onerror) {
      ...
      if (context.inwork == true) {
        if (isFunction(onerror)) onerror("Connection already in work");
